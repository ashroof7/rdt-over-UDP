...
#define PLP 0.1 // packet loss probability
#define MAX_cwnd 50
...
int win_base = 0; // base of the moving window
int ssthreshold = (MAX_cwnd/2);
int prev_cwnd = MAX_cwnd;//used to keep the value of the last cwnd on filling the buffer [0--> N - prev_cwnd]
...
int timers_status[MAX_SEQ_N]; // boolean if timer is acive = 1,2 and 0 is not active ...
// 1 indicates that the packet to be resent is a data_pkt. 2 indicates ACK pkt.
int timer_n_index ; // largest index in the timers array
...
void timer_handler(int sig) {
	...
	for(i = 0; i < MAX_SEQ_N; i++){
		local_timer = clock();
		if(timers_status[i]>0){ // recall 0 = notset   -1 not intialized   1 pkt_timer    2 ack_timer
			timers[i] -= (timeval2long(timeout_tv) + ((float) local_timer)/CLOCKS_PER_SEC*1000000);

			if (timers[i] <= 0){
				// re-send packet and update timer
				int temp =  sendto(worker_sock, (void *)&(packet_buff[i]), (timers_status[i]==1)?sizeof(pkt_t):sizeof(ack_t),
...
void hand_shake(){
	...
	timers_status[0] = 2; // quick and dirty sol: 2 indicates that the packet to resent is ack_t
