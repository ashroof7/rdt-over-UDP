...
void parse_response() {
    ...
    recvfrom(socket_fd, &recv_ack, sizeof(recv_ack), 0, (struct sockaddr*) &server_addr, &server_addr_len);
    ...
    while (1){
        ...
        sendto(socket_fd, &ack, sizeof(ack_t), 0, (struct sockaddr*) &server_addr, server_addr_len);

        recvfrom(socket_fd, &pkt, sizeof(pkt_t), 0, (struct sockaddr*) &server_addr, &server_addr_len);

        if (pkt.len != sizeof(ack_t))    // received packet is the 1st data packet
            break;
    ...
    while(1){
        ...
        if (pkt_index < base){
            ...
            if(filesize <= 0){ // why the < ---> if the last packet has data size < PKT_DATA_SIZE
